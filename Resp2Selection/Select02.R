...
updatePlot <- function(h,...) {
	r <- seq(1,floor(svalue(N)/svalue(sg1)/svalue(k)),1)
	g <- floor(svalue(N)/svalue(k)/r)
	alpha <- svalue(sg1)/g
	z <- dnorm(qnorm(1-alpha))
	i <- z/alpha
	rho <- sqrt(svalue(sigmaG2) / (svalue(sigmaG2) + svalue(sigmaGl2)/svalue(k) + svalue(sigmaE2)/svalue(k)/r))
	R <- i*rho
...
N <- gspinbutton(from=100, to=10000, by=100, value=1000, handler = updatePlot) # Total capacity
...
window <- gwindow("Response to selection with several locations")
