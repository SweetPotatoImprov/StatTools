        ...
        if (exists("CRW", where=datos)==1 & exists("NCRW", where=datos)==1)
          datos$RAUX <- apply(cbind(datos$CRW, datos$NCRW), 1, sum, na.rm=T) else
            if (exists("TRW", where=datos)==1)
              datos$RAUX <- datos$TRW else
                if (exists("RYTHA", where=datos)==1)
                  datos$RAUX <- datos$RYTHA else
                    if (exists("CRW", where=datos)==1)
                      datos$RAUX <- datos$CRW else
                        if (exists("CYTHA", where=datos)==1)
                          datos$RAUX <- datos$CYTHA
...
if (exists("RFR", where=datos)==1 & exists("CRW", where=datos)==1 & exists("NCRW", where=datos)==1 & exists("DMD", where=datos)==1
	...
	if (dim(subset(datos, abs(RFR-apply(cbind(CRW, NCRW), 1, sum, na.rm=T)*(DMD/DMF)/(VW*DMVD/DMVF))>1e-10))[1]>0){
		cat("\n","Root foliage ratio (RFR) is different from (CRW+NCRW)*(DMD/DMF)/(VW*DMVD/DMVF)*100:","\n")
		subset(datos, abs(RFR-apply(cbind(CRW, NCRW), 1, sum, na.rm=T)*(DMD/DMF)/(VW*DMVD/DMVF))>1e-10)
	}

## Outliers detection based on interquartile range and values out of range

if (exists("NOPE", where=datos)==1)
	if (dim(subset(datos, NOPE < 0))[1]>0){
...
if (exists("NONC", where=datos)==1)
	if (dim(subset(datos, NONC < 0))[1]>0){
...
if (exists("NCRW", where=datos)==1)
	if (dim(subset(datos, NCRW < 0))[1]>0){
...
if (exists("DMVF", where=datos)==1)
	if (dim(subset(datos, DMVF < 0))[1]>0){
...
if (exists("DMVD", where=datos)==1)
	if (dim(subset(datos, DMVD < 0))[1]>0){
...
if (exists("PROT", where=datos)==1)
	if (dim(subset(datos, PROT < 0))[1]>0){
...
if (exists("STAR", where=datos)==1)
	if (dim(subset(datos, STAR < 0))[1]>0){
...
if (exists("FRUC", where=datos)==1)
	if (dim(subset(datos, FRUC < 0))[1]>0){
...
if (exists("GLUC", where=datos)==1)
	if (dim(subset(datos, GLUC < 0))[1]>0){
...
if (exists("SUCR", where=datos)==1)
	if (dim(subset(datos, SUCR < 0))[1]>0){
...
if (exists("MALT", where=datos)==1)
	if (dim(subset(datos, MALT < 0))[1]>0){
...
if (exists("ACRW", where=datos)==1)
	if (dim(subset(datos, ACRW < 0))[1]>0){
...
if (exists("NRPP", where=datos)==1)
	if (dim(subset(datos, NRPP < 0))[1]>0){
...
if (exists("CI", where=datos)==1)
	if (dim(subset(datos, CI < 0 | CI > 100))[1]>0){
		cat("\n","Out of range values for commercial index (CI):","\n")
		subset(datos, CI < 0 | CI > 100)
	}
...
if (exists("HI", where=datos)==1)
	if (dim(subset(datos, HI < 0 | HI > 100))[1]>0){
		cat("\n","Out of range values for harvest index (HI):","\n")
		subset(datos, HI < 0 | HI > 100)
	}
...
if (exists("SHI", where=datos)==1)
	if (dim(subset(datos, SHI < 0 | SHI > 100))[1]>0){
		cat("\n","Out of range values for harvest sowing index (SHI):","\n")
		subset(datos, SHI < 0 | SHI > 100)
	}
...
if (exists("BIOM", where=datos)==1)
	if (dim(subset(datos, BIOM < 0))[1]>0){
...
if (exists("DM", where=datos)==1)
	if (dim(subset(datos, DM < 0 | DM > 100))[1]>0){
		cat("\n","Out of range values for storage root dry matter content (DM):","\n")
		subset(datos, DM < 0 | DM > 100)
	}
...
if (exists("DMFY", where=datos)==1)
	if (dim(subset(datos, DMFY < 0))[1]>0){
...
if (exists("DMRY", where=datos)==1)
	if (dim(subset(datos, DMRY < 0))[1]>0){
...
if (exists("RFR", where=datos)==1)
	if (dim(subset(datos, RFR < 0 | RFR > 100))[1]>0){
		cat("\n","Out of range values for root foliage ratio (RFR):","\n")
		subset(datos, RFR < 0 | RFR > 100)
	}
